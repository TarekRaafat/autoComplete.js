...
### v11.0 ✨
- 🧹 Removed:
  - `placeHolder` API property to be replaced with `attrs` (Thanks 👍 @mikob @folknor) #10 #11 #228
  - Redundant data fetch failure check at the `start` stage
...
- 📣 SHOUT-OUT: A massive appreciation for @folknor for being the most helpful and supportive member of the `autoComplete.js` community
