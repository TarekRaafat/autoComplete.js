(function (global, factory) {
   ...
   */ const mark = (value, cls)=>create("mark", _extends$3({
          innerHTML: value
      }, typeof cls === "string" && {
          class: cls
      })).outerHTML;
   ...
   *
   * @param {Object} ctx - autoComplete.js context
   */ const getData = (ctx, query)=>_async_to_generator$2(function*() {
          const { data } = ctx;
          if (data.cache && data.store) return;
          ctx.feedback = data.store = typeof data.src === "function" ? yield data.src(query) : data.src;
          /**
   ...
   */ const feedback = (ctx, index)=>{
      ctx.feedback.selection = _extends$2({
          index
   ...
   */ const render = (ctx)=>{
      ...
      if (matches.length || resultsList.noResults) {
          ...
          results.forEach((result, index)=>{
              ...
              const element = create(resultItem.tag, _extends$2({
   ...
   */ const addEvents = (ctx)=>{
      ...
      const publicEvents = ctx.events = _extends$1({
          input: _extends$1({}, events && events.input)
      }, ctx.resultsList && {
          list: events ? _extends$1({}, events.list) : {}
      });
   ...
   */ function init(ctx) {
      return _async_to_generator(function*() {
          ...
          create(ctx.input, _extends({
              "aria-controls": resultsList.id,
              "aria-autocomplete": "both"
          }, placeHolder && {
              placeholder: placeHolder
          }, !ctx.wrapper && _extends({}, parentAttrs)));
          // Create wrapper element
          if (ctx.wrapper) ctx.wrapper = create("div", _extends({
              around: ctx.input,
              class: ctx.name + "_wrapper"
          }, parentAttrs));
          if (resultsList) // Create new list element
          ctx.list = create(resultsList.tag, _extends({
              dest: [
          ...
          if (ctx.data.cache) yield getData(ctx);
          /**
